Feature: Invite link building (issue #179)
  ...
  js/facturen/clients-app.js turns into the link she copies and shares
  herself — see js/facturen/client-invite-link.js. Loading that link is a
  ...
  link-preview crawler; no /activate/ bridge page is needed anymore. It
  points at /invite/, a dedicated page (reusing the account page's Alpine
  component) so link previews show an invite-specific title instead of the
  generic "My bookings" one /account/ carries.
  ...
  Scenario Outline: The invite link points at the client's preferred language
    ...
    Examples:
      | lang | link                                                |
      | en   | https://gatoweb.nl/en/invite/?invite=abc123         |
      | nl   | https://gatoweb.nl/nl/invite/?invite=abc123         |
      | pt   | https://gatoweb.nl/pt/invite/?invite=abc123         |
      | fr   | https://gatoweb.nl/en/invite/?invite=abc123         |
  ...
  Scenario: A trailing slash on the site URL doesn't produce a double slash
    ...
    And a fresh invite token is "abc123"
    When the invite link is built
    Then the invite link is "https://gatoweb.nl/en/invite/?invite=abc123"
