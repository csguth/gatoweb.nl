...
## Procedure: Closing an Issue / Marking Done

**Never close a GitHub issue or move its board item to `Done` just because the code merged into `staging`.** This repo deploys `staging` and `main` separately (see `deploy-staging-cloudflare.yml` / `deploy-*-cloudflare.yml` for `main`), and `staging` is regularly ahead of `main`. An issue is only truly resolved once its commit has shipped to **production** (`main`).

Before closing an issue or setting Status to Done:
1. Identify the commit(s) that implement the fix (e.g. via `git log --oneline` or the PR merge commit).
2. Verify it's actually in `main`, not just `staging`:
   ```
   git fetch origin
   git merge-base --is-ancestor <commit-sha> origin/main; echo $LASTEXITCODE   # 0 = yes, in main
   ```
3. Only if that check passes (exit code 0), close the issue and set Status to Done.
4. If the work is only in `staging`, leave/set the issue Status to `In Progress` (not `Todo`, not `Done`) and note in a comment that it's deployed to staging but pending a `main` release.

If you mistakenly close/Done an issue that turns out to be staging-only, reopen it and revert Status to `In Progress` immediately, and tell the user what happened.
